...
int handle(char* ch) {
    ...
    newImagePath = suffix == "" ? newImagePrefix + flipStr : newImagePrefix + flipStr + "." + suffix;
    ...
    {
        if (newImagePrefix[newImagePrefix.size() - 1 - i] != flipStr[flipStr.size() - 1 - i]) {
            containsflipStr = false;
            break;
        }
    }
    if (containsflipStr) {
        newImagePath = suffix == "" ? newImagePrefix.substr(0, newImagePrefix.size() - flipStr.size()) : newImagePrefix.substr(0, newImagePrefix.size() - flipStr.size()) + "." + suffix;
        if (isFileExists(newImagePath)) {
            std::cout << newImagePath << " already exists." << std::endl;
            system("pause");
            return 3;
    ...
    if (image.channels() < 3) {
        ...
        default: {
            std::cout << imagePath << " has unknown image type." << std::endl;
            system("pause");
            break;
        }
        }
    ...
    /*
    int i = 1;
    while (isFileExists(newImagePath)) {
        newImagePath = newImagePrefix + flipStr + std::to_string(i) + "." + suffix;
        ++i;
    }
    */
